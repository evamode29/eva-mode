...
export async function POST(request: Request) {
  try {
    // Supabase server client is loaded only when the API actually runs.
    // This prevents NEXT_PUBLIC_SUPABASE_URL from being required
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: "یکی از اطلاعات واردشده بیش از حد مجاز است.",
        },
        { status: 400 }
      );
    }

    if (!validMobile(customerMobile)) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: "سبد خرید خالی یا نامعتبر است.",
        },
        { status: 400 }
      );
    }
    ...
    for (const raw of body.items) {
      if (!raw || typeof raw !== "object") {
        return NextResponse.json(
          {
            success: false,
            error: "اطلاعات یکی از محصولات نامعتبر است.",
          },
          { status: 400 }
        );
      }
      ...
      if (nextQuantity > 20) {
        return NextResponse.json(
          {
            success: false,
            error: "تعداد یک محصول بیشتر از حد مجاز است.",
          },
          { status: 400 }
        );
      }
    ...
    if (error || !data?.[0]) {
      ...
      const userMessage = message.includes("insufficient stock")
        ? "موجودی یکی از محصولات کافی نیست."
        : message.includes("product unavailable")
          ? "یکی از محصولات دیگر قابل سفارش نیست."
          : message.includes("invalid variant")
